            ...
            if len(continuous_cols) == 0:
                ...
                if feature_mask:
                    mask = [
                        ...
                        'x19_Never',
                        'x19_Rarely',
                        'x20_A Little Important',
                        'x20_Not Important',
                        'x20_Pretty Important',
                        'x20_Very Important',
                        'x21_Above Average',
                        'x21_Average',
                        'x22_Average',
                    ...
                    x_t = pd.concat([pd.DataFrame(scaled, columns=cont), pd.DataFrame(
                        dummies, columns=ohe.get_feature_names())], axis=1)
                    x_t = x_t[mask]
                    ...
                    x_val = pd.concat([pd.DataFrame(sc, columns=cont), pd.DataFrame(
                        dums, columns=ohe.get_feature_names())], axis=1)
